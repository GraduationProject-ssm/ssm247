...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1gn8XeNE2J?p=45)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本缪斯乐器购物网站也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让医生管理信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，缪斯乐器购物网站的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，缪斯乐器购物网站是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此缪斯乐器购物网站为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
缪斯乐器购物网站的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
|24|title|String|标题|是|
|25|picture|String|图片|是|
|26|content|String|内容|是|
表4.5订单表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|27|addtime|Date|创建时间|是|
|28|orderid|String|订单编号|是|
|29|tablename|String|商品表名|是|
|30|userid|Integer|用户id|是|
|31|goodid|Integer|商品id|是|
|32|goodname|String|商品名称|是|
|33|picture|String|商品图片|是|
|34|buynumber|Integer|购买数量|是|
|35|price|float|价格/积分|是|
|36|discountprice|float|折扣价格|是|
|37|total|float|总价格/总积分|是|
|38|discounttotal|float|折扣总价格|是|
|39|type|Integer|支付类型|是|
|40|status|String|状态|是|
|41|address|String|地址|是|
表4.6商品分类表
...
|43|shangpinfenlei|String|商品分类|是|
表4.7商品信息表
...
|45|shangpinbianhao|String|商品编号|是|
|46|shangpinmingcheng|String|商品名称|是|
|47|shangpinfenlei|String|商品分类|是|
|48|shuliang|String|数量|是|
|49|pinpai|String|品牌|是|
|50|guige|String|规格|是|
|51|xiangqing|String|详情|是|
|52|fengmian|String|封面|是|
|53|clicktime|datetime|最近点击时间|是|
|54|clicknum|Integer|点击次数|是|
|55|price|float|价格|是|
表4.8收藏表表
...
|57|userid|Integer|用户id|是|
|58|refid|Integer|收藏id|是|
|59|tablename|String|表名|是|
|60|name|String|收藏名称|是|
|61|picture|String|收藏图片|是|
表4.9用户表
...
|63|yonghuzhanghao|String|用户账号|是|
|64|yonghuxingming|String|用户姓名|是|
|65|mima|String|密码|是|
|66|xingbie|String|性别|是|
|67|lianxidianhua|String|联系电话|是|
|68|dianziyouxiang|String|电子邮箱|是|
|69|money|float|余额|是|
表4.10用户表表
...
图5.1 用户信息管理页面
### 5.2 商品类型管理
如图5.2显示的就是商品类型管理页面，此页面提供给管理员的功能有：查看已发布的商品类型数据，修改商品类型，商品类型作废，即可删除。
